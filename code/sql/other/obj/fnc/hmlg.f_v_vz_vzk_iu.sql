DROP FUNCTION IF EXISTS hmlg.f_v_vz_vzk_iu(character varying) CASCADE;
CREATE OR REPLACE
FUNCTION hmlg.f_v_vz_vzk_iu(
...
BEGIN
  ...
  SELECT j.*
    INTO r_new
    FROM json_populate_record(null::hmlg.v_vz_vzk, j_rec) j;
  ...
  IF vb_ins THEN
    INSERT INTO hmlg.vz_vzk (vz_id, vzk_id)
      VALUES (r_new.vz_id, r_new.vzk_id);
...
$$
  LANGUAGE 'plpgsql';
ALTER FUNCTION hmlg.f_v_vz_vzk_iu(character varying) OWNER TO postgres;
