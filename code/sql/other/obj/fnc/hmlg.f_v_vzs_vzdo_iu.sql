DROP FUNCTION IF EXISTS hmlg.f_v_vzs_vzdo_iu(character varying) CASCADE;
CREATE OR REPLACE
FUNCTION hmlg.f_v_vzs_vzdo_iu(
...
DECLARE
  ...
  r_new hmlg.v_vzs_vzdo;
  r_old hmlg.v_vzs_vzdo;
...
BEGIN
  ...
  SELECT j.*
    INTO r_new
    FROM json_populate_record(null::hmlg.v_vzs_vzdo, j_rec) j;
  ...
  IF vb_ins THEN
    INSERT INTO hmlg.vzs_vzdo (vzs_id, vzdo_id)
      VALUES (r_new.vzs_id, r_new.vzdo_id);
...
$$
  LANGUAGE 'plpgsql';
ALTER FUNCTION hmlg.f_v_vzs_vzdo_iu(character varying) OWNER TO postgres;
