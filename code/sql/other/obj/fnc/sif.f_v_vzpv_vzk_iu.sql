DROP FUNCTION IF EXISTS sif.f_v_vzpv_vzk_iu(character varying) CASCADE;
CREATE OR REPLACE
FUNCTION sif.f_v_vzpv_vzk_iu(
...
DECLARE
  ...
  r_new sif.v_vzpv_vzk;
  r_old sif.v_vzpv_vzk;
...
BEGIN
  ...
  SELECT j.*
    INTO r_new
    FROM json_populate_record(null::sif.v_vzpv_vzk, j_rec) j;

-- sif.vozilo_karoserija
  IF r_new.vzk_oznaka IS NOT NULL AND r_new.vzk_naziv IS NOT NULL THEN
    ...
    IF r_new.vzv_id IS NULL THEN
      vi_id := sif.f_vozilo_karoserija_iu(vc_rec);
      r_new.vzk_id := vi_id;
      vi_row_count := 1;
    ELSE
      vi_row_count := sif.f_vozilo_karoserija_iu(vc_rec);
    END IF;
  END IF;
...
$$
  LANGUAGE 'plpgsql';
ALTER FUNCTION sif.f_v_vzpv_vzk_iu(character varying) OWNER TO postgres;
