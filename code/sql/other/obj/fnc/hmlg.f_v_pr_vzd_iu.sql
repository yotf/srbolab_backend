DROP FUNCTION IF EXISTS hmlg.f_v_pr_vzd_iu(character varying) CASCADE;
CREATE OR REPLACE
FUNCTION hmlg.f_v_pr_vzd_iu(
...
BEGIN
  ...
  SELECT j.*
    INTO r_new
    FROM json_populate_record(null::hmlg.v_pr_vzd, j_rec) j;

  SELECT COUNT(*)=0
    INTO vb_ins
    FROM hmlg.v_pr_vzd t
    WHERE t.pr_id=r_new.pr_id
      AND t.vzd_id=r_new.vzd_id;

  IF vb_ins THEN
    INSERT INTO hmlg.pr_vzd (pr_id, vzd_id)
      VALUES (r_new.pr_id, r_new.vzd_id);
...
EXCEPTION
  ...
  WHEN unique_violation THEN
    IF regexp_match(sqlerrm, 'prus_pk', 'i') IS NOT NULL THEN
      RAISE unique_violation USING MESSAGE = 'Takav ID dokumenta već postoji!';
    ELSE
...
$$
  LANGUAGE 'plpgsql';
ALTER FUNCTION hmlg.f_v_pr_vzd_iu(character varying) OWNER TO postgres;
