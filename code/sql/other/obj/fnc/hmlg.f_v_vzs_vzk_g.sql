DROP FUNCTION IF EXISTS hmlg.f_v_vzs_vzk_g(character varying) CASCADE;
CREATE OR REPLACE
FUNCTION hmlg.f_v_vzs_vzk_g(
                            pc_rec character varying DEFAULT NULL::character varying
                           )
  RETURNS SETOF hmlg.v_vzs_vzk AS
$$
DECLARE
...
BEGIN
  ...
  FOR r_rec IN SELECT v.*
                 FROM hmlg.v_vzs_vzk v
                   CROSS JOIN (SELECT r.* from json_populate_record(null::sif.v_vzs_vzk, j_rec) r) j
...
$$
  LANGUAGE 'plpgsql';
ALTER FUNCTION hmlg.f_v_vzs_vzk_g(character varying) OWNER TO postgres;
