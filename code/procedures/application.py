      ...
      if row.afo_id not in apps[row.aap_id].forms:
        if row.afo_dostupna == 'y' or pb_disabled:
          apps[row.aap_id].forms[row.afo_id] = {
              ...
              row.afo_dostupna == 'y',
              'useractions':
              (js.loads(row.arf_akcije_d) if row.arf_akcije_d else {}),
          }
          ...
          if apps[row.aap_id].forms[row.afo_id].enabled:
            if hasattr(apps[row.aap_id].forms[row.afo_id].tables, "actions"
                       ) and apps[row.aap_id].forms[row.afo_id].tables.actions:
              for vil_ActIdx, vcl_FormAction in enumerate(
                  apps[row.aap_id].forms[row.afo_id].tables.actions):
                ...
                dcl_Act['enabled'] = vcl_FormAction in apps[row.aap_id].forms[
                    row.afo_id].useractions.get('actions', [])
                apps[row.aap_id].forms[
                    row.afo_id].tables.actions[vil_ActIdx] = dcl_Act
