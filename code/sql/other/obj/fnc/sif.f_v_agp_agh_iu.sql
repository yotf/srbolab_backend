DROP FUNCTION IF EXISTS sif.f_v_agp_agh_iu(character varying) CASCADE;
CREATE OR REPLACE
FUNCTION sif.f_v_agp_agh_iu(
...
BEGIN
  ...
  SELECT j.*
    INTO r_new
    FROM json_populate_record(null::sif.v_agp_agh, j_rec) j;

-- sif.ag_homologacija
  IF r_t.agh_oznaka IS NOT NULL THEN
    ...
    IF r_new.agh_id IS NULL THEN
      vi_id := sif.f_ag_homologacija_iu(vc_rec);
      r_new.agh_id := vi_id;
      vi_row_count := 1;
    ELSE
      vi_row_count := sif.f_ag_homologacija_iu(vc_rec);
    END IF;
  END IF;
...
$$
  LANGUAGE 'plpgsql';
ALTER FUNCTION sif.f_v_agp_agh_iu(character varying) OWNER TO postgres;
