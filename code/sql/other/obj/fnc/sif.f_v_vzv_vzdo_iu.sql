DROP FUNCTION IF EXISTS sif.f_v_vzv_vzdo_iu(character varying) CASCADE;
CREATE OR REPLACE
FUNCTION sif.f_v_vzv_vzdo_iu(
...
DECLARE
  ...
  r_new sif.v_vzv_vzdo;
  r_old sif.v_vzv_vzdo;
...
BEGIN
  ...
  SELECT j.*
    INTO r_new
    FROM json_populate_record(null::sif.v_vzv_vzdo, j_rec) j;

-- sif.vozilo_dod_oznaka
  IF r_new.vzdo_oznaka IS NOT NULL AND r_new.vzdo_naziv IS NOT NULL THEN
    SELECT row_to_json(t)::character varying
      INTO vc_rec
      FROM (
            SELECT r_new.vzdo_id,
                   r_new.vzdo_oznaka,
                   r_new.vzdo_naziv
           ) t;
    IF r_new.vzdo_id IS NULL THEN
      vi_id := sif.f_vozilo_dod_oznaka_iu(vc_rec);
      r_new.vzdo_id := vi_id;
      vi_row_count := 1;
    ELSE
      vi_row_count := sif.f_vozilo_dod_oznaka_iu(vc_rec);
    END IF;
  END IF;
...
$$
  LANGUAGE 'plpgsql';
ALTER FUNCTION sif.f_v_vzv_vzdo_iu(character varying) OWNER TO postgres;
